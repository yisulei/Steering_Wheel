...
void Base_Motor_Init(void)
{
	...
	for(i=0;i<3;i++)
	{
		...
		Robo_Base.Motor_Turn[i].MotorPos_PID_Init			(1.0f, 0.0012f,   32.0f, 	 5000,  6000, 	5000,		0);
		Robo_Base.Motor_Turn[i].MotorSpeed_PID_Init		(5.0f, 	 0.05f, 	0.06f, 	10000,  6000, 	300000,	0);
		//电机初始化
		Robo_Base.Motor_Turn[i].Motor_PosInit();
	}
	Robo_Base.Motor_Move[2].Motor_PID_Init	 			(20.0f,	 0.06f,		0.05f,	10000,	5000,		5000,		0);
}
/**
...
{
	Robo_Base.Vel_X = Vel_X;
	Robo_Base.Vel_Y = Vel_Y;
	Robo_Base.Vel_W = Vel_W;
...
static void ThreeSteering_MoveIK(float Vel_Y,float Vel_X,float Vel_W)
{
	...
	Motor_Tar[0] = speed[F] /3 * 19;
	Motor_Tar[1] = speed[LB] /3 *19;
	Motor_Tar[2] = speed[RB] /3 *19;
...
static void MotorTarget_Set()
{
	for (int num = 0;num < num_wheel;num++)
	{
		...
		if(abs(Robo_Base.Motor_Turn[num].Pos_Info.Abs_Angle - Robo_Base.Motor_Turn[num].Tar_Pos) <= 250)	
