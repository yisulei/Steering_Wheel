...
{
	//PID 初始化
	Robo_YunAndArm.Motor_Arm.MotorPos_PID_Init			(1.0f, 0.0012f,  	 	 32,	 	500,	6000,		5000,		0);
	Robo_YunAndArm.Motor_Arm.MotorSpeed_PID_Init		(5.0f,	 0.05f,		0.06f, 	10000,	6000,		300000,	0);
	Robo_YunAndArm.Motor_Wrist.MotorPos_PID_Init		(1.0f, 0.0012f,  	 	 32,	 	500,	6000,		5000,		0);
	Robo_YunAndArm.Motor_Wrist.MotorSpeed_PID_Init	(5.0f,	 0.05f,		0.06f, 	10000,	6000,		300000,	0);
	//电机初始化
	Robo_YunAndArm.Motor_Arm.Motor_PosInit(0);
	Robo_YunAndArm.Motor_Wrist.Motor_PosInit(0);
}

float Arm_offset_moto(int32_t Tar_Pos) //大臂电机 重力补偿
{
	if(Tar_Pos > -70000) return -3500.0f;
	else return 0;
}
